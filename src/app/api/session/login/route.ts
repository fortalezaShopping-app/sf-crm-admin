...
import { isSameOriginRequest, readJsonObject, readRequestBody, RequestInputError } from '@/lib/request-security';
...
async function readCredentials(request: Request, isFormSubmission: boolean) {
  ...
  const formData = new URLSearchParams(new TextDecoder().decode(await readRequestBody(request)));
